...
const BATCH_MIN = 5;
const BATCH_MAX = 15;
const INGESTION_DURATION = parseInt(process.env.INGESTION_DURATION, 10) || 60; // seconds
...
module.exports = function strategyCurrentSlow() {
  const now = Date.now() * 1e6;
  const durationNs = INGESTION_DURATION * 1e9;
  let logs = [];
  for (let i = 0; i < LOG_COUNT; i++) {
    const base = now + Math.floor((i / LOG_COUNT) * durationNs);
    const jitter = Math.floor((Math.random() - 0.5) * 400 * 1e6);
    const timestamp = base + jitter;
  ...
  const delay = Math.floor((INGESTION_DURATION * 1000) / allBatches.length);
  ...
  for (const batchLogsArr of allBatches) {
    ...
    batches.push({ logs: batchLogsArr, delay, meta: { strategy: 'current_slow', labels: { ...config.getLabels(), job, host }, ingestionDuration: INGESTION_DURATION } });
    batchIdx++;
  }
  return { batches, meta: { strategy: 'current_slow', delay, ingestionDuration: INGESTION_DURATION } };
}; 