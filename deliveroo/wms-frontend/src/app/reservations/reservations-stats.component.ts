    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
      ...
      <div class="card p-6">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Unique Customers</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ getUniqueCustomers() }}</p>
          </div>
        </div>
      </div>
    </div>
...
export class ReservationsStatsComponent {
  ...
  getUniqueCustomers(): number {
    const uniqueCustomers = new Set(this.reservations().map(r => r.customerId));
